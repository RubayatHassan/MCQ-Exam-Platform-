  ...
  asyncRoute(async (req, res) => {
    ...
    if (input.email) {
      const existing = await prisma.user.findFirst({
        where: { email: input.email, id: { not: req.user!.sub } },
      });
      if (existing) return fail(res, 'Email is already in use', 409);
    }
...
app.use((err: unknown, _req: Request, res: Response, _next: NextFunction) => {
  if (err instanceof z.ZodError) return fail(res, 'Validation failed', 422);
  console.error(err);
  return fail(res, 'Internal server error', 500);
});
if (process.env.NODE_ENV !== 'test')
  app.listen(port, () => console.log(`MCQ API listening on ${port}`));
export { app, prisma };
